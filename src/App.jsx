...
function Row({ label, val, color, big }) {
  return (
    <div style={{ marginBottom: 12 }}>
      <div style={{ fontSize: 10, color, textTransform: "uppercase", letterSpacing: "0.1em" }}>{label}</div>
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: 36 }}>
        <div style={{ fontSize: 10, letterSpacing: "0.25em", textTransform: "uppercase", color: "#7a8494", marginBottom: 10 }}>
        ...
        <p style={{ color: "#a0a8b4", marginTop: 10, fontSize: 13, lineHeight: 1.7, maxWidth: 620 }}>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 32, flexWrap: "wrap" }}>
        ...
        <div style={{
          ...
          padding: "8px 14px", fontSize: 11, fontFamily: "monospace", color: "#9098a8",
        }}>
      ...
      <div style={{ display: "flex", gap: 32, marginBottom: 36, flexWrap: "wrap" }}>
        {[
          ...
          <div key={ctrl.label} style={{ flex: 1, minWidth: 220 }}>
            <label style={{ display: "block", fontSize: 10, letterSpacing: "0.2em", textTransform: "uppercase", color: "#7a8494", marginBottom: 8 }}>
              {ctrl.label}
            </label>
      ...
      <div style={{
        ...
        {[
          ...
          <div key={item.label}>
            <div style={{ fontSize: 10, color: "#7a8494", letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: 4 }}>{item.label}</div>
            <div style={{ fontFamily: "monospace", fontSize: 22, color: item.color }}>{item.val}<span style={{ fontSize: 12, color: "#9098a8" }}>/yr</span></div>
            <div style={{ fontSize: 11, color: "#7a8494", marginTop: 2 }}>{item.sub}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16, marginBottom: 40 }}>
        {milestoneData.map(({ years, invested, portu, t212, gap }) => (
          <div key={years} style={{
            ...
            <div style={{ fontSize: 10, letterSpacing: "0.2em", textTransform: "uppercase", color: "#7a8494", marginBottom: 18 }}>
              After {years} years
            </div>
            <Row label="Total invested" val={fmt(invested)} color="#8a919e" />
            <Row label="Portu" val={fmt(portu)} color="#7a9cc4" big />
            <div style={{ fontSize: 10, color: "#7a8494", marginTop: -8, marginBottom: 12 }}>
            ...
            <div style={{ borderTop: "1px solid #1e2230", paddingTop: 14, marginTop: 4 }}>
              ...
              <div style={{ fontSize: 11, color: "#7a8494", marginTop: 3 }}>
      ...
      <div style={{ background: "#0d0f14", border: "1px solid #1a1e28", borderRadius: 12, padding: "24px 8px 16px" }}>
        <div style={{ fontSize: 10, letterSpacing: "0.2em", textTransform: "uppercase", color: "#7a8494", paddingLeft: 16, marginBottom: 20 }}>
        ...
        <ResponsiveContainer width="100%" height={310}>
          <AreaChart data={chartData} margin={{ top: 4, right: 20, left: 0, bottom: 0 }}>
            ...
            <XAxis dataKey="year" tick={{ fill: "#6a7484", fontSize: 11 }} tickLine={false} axisLine={false}
              label={{ value: "Years", fill: "#6a7484", fontSize: 10, position: "insideBottomRight", offset: -8 }} />
            <YAxis
              tickFormatter={v => v >= 1_000_000 ? `${(v / 1_000_000).toFixed(1)}M` : `${(v / 1000).toFixed(0)}k`}
              tick={{ fill: "#6a7484", fontSize: 11 }} tickLine={false} axisLine={false} width={52} />
            <Tooltip content={<CustomTooltip />} />
            <Legend wrapperStyle={{ fontSize: 12, color: "#9098a8", paddingTop: 14 }} />
      ...
      <p style={{ fontSize: 11, color: "#6a7484", lineHeight: 1.8, borderTop: "1px solid #1e2230", paddingTop: 16, marginTop: 24 }}>
